 ...
 *          type == LV_STREAM_CMD_STORAGE_RECORD_BY_FILE时，
 ...
 * @retval  0 : Success.
 */
typedef int (*lv_on_push_streaming_cmd_cb)(int service_id, lv_on_push_streaming_cmd_type_e cmd, unsigned int timestamp_ms);
...
typedef struct lv_config_s {
    ...
    /* 码流数量配置 */
    /* 直播源目前只支持1路，多客户端观看时，客户端观看的码流是一致的。
 ...
 * @return lv_error_e
 */
int lv_linkkit_adapter(lv_linkkit_adapter_type_s type, const lv_linkkit_adapter_property_s *in);
