...
int iotx_dm_subdev_connect(_IN_ int devid)
{
    ...
    char *p_pk_dn = NULL;
    int pk_dn_len = 0;
    const char pk_dn_fmt[] = "[{\"ProductKey\":\"%s\",\"DeviceName\":\"%s\"}]";
    dm_mgr_dev_node_t *search_node = NULL;
    ...
    pk_dn_len = strlen(pk_dn_fmt) + PRODUCT_KEY_MAXLEN + DEVICE_NAME_MAXLEN;
    p_pk_dn = DM_malloc(pk_dn_len);
    if (!p_pk_dn) {
        return DM_MEMORY_NOT_ENOUGH;
    }

    HAL_Snprintf(p_pk_dn, pk_dn_len, pk_dn_fmt, search_node->product_key, search_node->device_name);

    res = dm_mgr_subdev_connect(devid, p_pk_dn, strlen(p_pk_dn));

    if (p_pk_dn) DM_free(p_pk_dn);
