                                   ...
                                   _OU_ char device_secret[DEVICE_SECRET_MAXLEN])
{
    ...
    {
        p_http_url = IMPL_LINKKIT_DYNREG_MALLOC(GUIDER_URL_LEN);
        if (!p_http_url)
        {
            sdk_err("no mem");
            return FAIL_RETURN;
        ...
        memset(p_http_url, '\0', GUIDER_URL_LEN);

        res = iotx_guider_get_dynamic_https_url(p_http_url, GUIDER_URL_LEN - 1);
        if (res != 0)
        {
            memset(p_http_url, '\0', GUIDER_URL_LEN);
        }

        sdk_info("kv http url:%s", p_http_url);
        only_use_one_time = 1;
    }

    url_len = strlen(url_format) + 1;
    if (p_http_url && strlen(p_http_url) > 0)
    {
        url_len += strlen(p_http_url);
    }
    else
    {
    ...
    if (NULL == url)
    {
        ...
        if (p_http_url)
        {
            IMPL_LINKKIT_DYNREG_FREE(p_http_url);
        }
    ...
    if (p_http_url && strlen(p_http_url) > 0)
    {
        HAL_Snprintf(url, url_len, url_format, p_http_url);
        IMPL_LINKKIT_DYNREG_FREE(p_http_url);
    }
    else
    {
