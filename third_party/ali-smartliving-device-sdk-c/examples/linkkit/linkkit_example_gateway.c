...
// for demo only
#define PRODUCT_KEY "a2sQxuXG1cx"
#define PRODUCT_SECRET "ElUhyA29337YQaxx"
#define DEVICE_NAME "gateway_device_x"
#define DEVICE_SECRET "zh1W6SRpT0QI5hA54EKilSCyt1epGXXX"

#define GATEWAY_YIELD_TIMEOUT_MS (200)

#define GATEWAY_SUBDEV_PERMIT_ADD_NUM (2)
#define GATEWAY_SUBDEV_MAX_NUM (20)
...
const iotx_linkkit_dev_meta_info_t subdevArr[GATEWAY_SUBDEV_MAX_NUM] = {
...
//And you can you gateway_addall_subdev add one sub dev
//ITM_MSG_ALL_SUBDEV_CONNECT is the new interface
...
//This is example for add one or more subdev
static int gateway_addall_subdev(int master_devid, iotx_linkkit_dev_meta_info_t *subdev_list, int subdev_num)
{
    int res = 0;

#ifdef DM_SUBDEV_NEW_CONNECT
    res = IOT_Linkkit_Report(master_devid, ITM_MSG_ALL_SUBDEV_CONNECT, subdev_list, sizeof(iotx_linkkit_dev_meta_info_t) * subdev_num);
    if (res == FAIL_RETURN)
    {
        gateway_err("add all subdev Failed\n");
    }
    else
    {
        gateway_info("add all subdev success\n");
    }
#endif
...
static int gateway_del_subdev(iotx_linkkit_dev_meta_info_t *subdev_mate)
{
    ...
    ret = IOT_Linkkit_Report(subdev_id, ITM_MSG_DELETE_TOPO, subdev_mate, sizeof(iotx_linkkit_dev_meta_info_t));
...
static int gateway_reset_subdev(iotx_linkkit_dev_meta_info_t *subdev_mate)
{
    ...
    //Here pk and dn of subdev is priorior than subdev id
    ret = IOT_Linkkit_Report(subdev_id, ITM_MSG_SUBDEV_RESET, subdev_mate, sizeof(iotx_linkkit_dev_meta_info_t));
    if (SUCCESS_RETURN != ret)
    {
...
static int gateway_query_subdev_id(iotx_linkkit_dev_meta_info_t *subdev_mate)
{
    ...
    subdev_id = IOT_Linkkit_Query(gateway_ctx->master_devid, ITM_MSG_QUERY_SUBDEV_ID, subdev_mate, sizeof(iotx_linkkit_dev_meta_info_t));
    if (subdev_id < 0)
    {
...
int linkkit_main(void *paras)
{
    ...
    while (1)
    {
        ...
        /* Add subdev */
        if (gateway_ctx->cloud_connected && gateway_ctx->permit_join && (gateway_ctx->subdev_index < GATEWAY_SUBDEV_PERMIT_ADD_NUM))
        {
            /* Add next subdev */
